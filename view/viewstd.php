...
// $program->close();
  ...
  while($zeile = $ergebnis->fetch_array()){
    ...
    $swgs84b_in = floatval($zeile['WGS84B']);
    $swgs84l_in  = floatval($zeile['WGS84L']);
...
// $progra =  $mysqli->query ("SELECT imei, ver, ID,pos FROM bdccio WHERE bfccio.imei = '$tpuser' ORDER BY ID DESC LIMIT 1");
 $progra =  $mysqli->query ("SELECT imei, ver, ID,pos FROM bfccio WHERE bfccio.imei = '$tpuser' ORDER BY ID DESC LIMIT 1");
...
// ------ Fehlerstatus
  $frage = "SELECT * FROM bfprojektstatus2_new WHERE PUSER = '$spuser';";
  $ergebnis = $mysqli->query($frage);
  while($zeile = $ergebnis->fetch_array()){
    $serr1 = $zeile['ERROR'];
    $serr2 = $zeile['TIMEOUT'];
...
if ($sesOK == 1){
  ...
  <nav class="navbar navbar-inverse">
    <div class="container">
      ...
      <ul class="nav navbar-nav">
        ...
        <li><a href="#">Map</a></li>
        <!-- <li class="active"><a href="../view/archive.php?WW=1&PUSER=".$spuser."&PID=\".$sid."\&TIME=\".$datazeit."\">Archive</a></li>  instead of # I added a link to map -->
          ...
          if ($sesAdmin == 1){ ?>
            <li class="dropdown">
              ...
              <ul class="dropdown-menu">
                ...
                <li><a href="../XYA/ADM.php?server=localhost&username=lanthan-01&db=lanthan_test&pass=7VrBi-.GXTZCF" target="_blank">Adminer</a></li>
              </ul>
            </li>           
            <?php
          }        
  ...
  if ($tpuser  == '359315076932424')  {

    $swgs84b= $swgs84b_in;
    $swgs84l  = $swgs84l_in;
    $error = "\n\r";
  } else {
    $error = "        <strong> &#177; ". $p1 ." Km </strong>". "\n\r";
  }
...
<?php
  ...
  if ($db_table == 'bftopass')
  {
    ...
    $frage = "SELECT * FROM bftopass WHERE ((ZEIT >$tstamp1) && (ZEIT<$tstamp2) && (PUSER = '$spuser')) ORDER BY ZEIT DESC LIMIT 25;";  
    $ergebnis = $mysqli->query($frage);
    $xold = 0;
    while($zeile = $ergebnis->fetch_array()){
      echo "  <tr>\n\r";
