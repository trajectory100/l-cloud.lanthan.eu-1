...
while($zeile = $projekte->fetch_array())
{
      ...
      if ($aktiv == '1')
      {
        ...
        if (strlen($zeile2['Dx']) > 15)        
        {
          ...
          { 
            ...
            {
              // $bferrD[$n] = ((int)$DxALVAL[$n] == (int)$zeile2[$DxVAR[$n]]);
              $bferrD[$n] = ((int)$DxALVAL[$n] == $bfD[$n]);
              // echo $datauser . ' : '. (int)$DxALVAL[$n] . ' :  '. $bfD[$n] . ' :  '. $n .  '<br>';
              if (($DxNOE[$n] == 'D') AND ($TWst == '1'))
              {
        ...
        $bfDerr = intval($bferrD[0]) .intval($bferrD[1]) .intval($bferrD[2]) .intval($bferrD[3]) .intval($bferrD[4]);
        ...
        $bfDerr = $bfDerr  .intval($bferrD[13]) .intval($bferrD[14]) .intval($bferrD[15]);
  ...
  // Abspeichern
  $insert = "INSERT INTO bfprojektstatus2 (ID, PUSER, ERROR, TIMEOUT, LASTDATA, AKTIV, DERR, A1ERR, A2ERR, TIME) VALUES";
  $insert = $insert ."(\"" .$pid ."\", \"" .$puser ."\", \"" .intval($bferr) ."\", \"" .intval($bftimeout) ."\", \"" .$datazeit ."\", \"" .$aktiv ."\", ";
  $insert = $insert ."\"" .$bDferr ."\", \"" .$bfA1err ."\", \"" .$bfA2err ."\", \"" .time() ."\")";
    ...
    if ($zeile2 != NULL)
    {
      ...
      if ($aktiv == '1')
      {
        ...
        if (strlen($zeile2['Dx']) > 15)        
        {
          for ($n = 1; $n <= 16; $n++)  
          { 
            $bfD[$n] = (int)substr($zeile2['Dx'],($n-1),1); //01110000001001110	
            //echo $bfD[$n];
            $bferrD[$n] = 0;
            $TWnoe = '-' .intval($TWst) .$DxNOE[$n];
            if (($DxEN[$n] == "1") && ($DxALERT[$n] == "1"))
