...
while ($zeile = $projini->fetch_array()) {
    ...
    while ($row1 = $all->fetch_array()) {
        ...
        while ($row2 =  $latest->fetch_array()) {
            ...
            $err1 = 1;
            $DX1cut = '0101110001000000';


            echo $puser .' = '. $err1 .' = '. $err2 .' = '.  $DX1 .' = '. $DX2 .' = '  . '<br>';

            if (($imei1 == $imei2) &&  ($err1 !==  $err2) && ($DX1cut !== $DX2cut) ) {
                $count = 1;
                $in = 0;
                    ...
                    while ($rowr = $result->fetch_array()) {
                        if (!empty($rowr['EMAIL'])) {
                            ...
                            if (!empty(stristr($ort, "Janneby"))) {
                                ...
                                $html_table2 .= "<p>E-Mail: <a href=\"mailto:notam.office@dfs.de\">notam.office@dfs.de</a></p>";
                                $html_table2 .="</div>";
                            } else {
                            }
