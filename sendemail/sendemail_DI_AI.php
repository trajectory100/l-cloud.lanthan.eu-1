          ...
          if  ($J<$min_AI10error ||  $J>$max_AI10error){
            ...
            if (in_array(1, $bferrD2)) {
                $err2 = 1;
                    ...
                    $count = 1;
                    $in = 0;
                    // echo $count . ' = ' .  $puser .' = ' .$ID. ' = ' .$imei1 . ' = '. $DX1 . ' = '. $errors_w1  .  ' = '. print_r( $err_keys_w1)  . '   : '.  date('j.n.Y.', $dattime1) . ' um ' . date('H:i:s', $dattime1) .'   ***  ' .$ID2. ' = '. $imei2  . ' = ' . $DX2 .' = ' .$errors_w2 . ' = '. print_r($err_keys_w2)  . '<br>';
                            if ($err1==1 && $err2==0) {
                                $in = 1;
                            ...
                            if ($in === 1) {
                                ...
                                LEFT JOIN bfuser ON (bfuser.ID = bfuserhasprojekt.userID) WHERE  bfprojektini.PUSER =  ' . "$puser" . ' ;');
                                    ...
                                    if (!empty($rowr['EMAIL'])) {
                                        ...
                                        $mail= 0;
                                        if (strpos($ort2, "Janneby") !== false)
                                            $subject = "Statusmeldung Janneby";
                                        else {
