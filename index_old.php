...
if ($sesOK == 1){
  ...
  <nav class="navbar navbar-inverse">
    <div class="container">
      ...
      <ul class="nav navbar-nav">
        <li class="active"><a href="https://test.lanthan.eu/index.php">Home</a></li>
          ...
          if ($sesAdmin == 1){ ?>
            <li class="dropdown">
              ...
              <ul class="dropdown-menu">
                ...
                <li><a href="./XYA/ADM450.php?server=rdbms.strato.de&username=lanthan-01&db=lanthan_test&pass=7VrBi-.GXTZCF" target="_blank">Adminer</a></li>
              </ul>
            </li>           
            <?php
          }        
  ...
  if (($sesSuperadmin == 1) || ($sesViewall == 1)){
    ...
    $frage = "SELECT * FROM bfprojektini AS bpi JOIN bfprojektstatus2 as bps WHERE bpi.AKTIV = '1' AND bpi.PUSER = bps.PUSER"; //alt listet nur Projekte die auch vom Status erfasst wurden 
  ...
  //  $frage = "SELECT * FROM bfuserhasprojekt AS bhp JOIN bfprojektini AS bpi JOIN bfprojektstatus as bps WHERE bhp.userID = '$sesUserid' AND bhp.projektiniID = bpi.ID AND bpi.AKTIV = '1' AND bpi.PUSER = bps.PUSER;";
    ...
    $frage = "SELECT * FROM bfuserhasprojekt AS bhp JOIN bfprojektini AS bpi JOIN bfprojektstatus2 as bps WHERE bhp.userID = '$sesUserid' AND bhp.projektiniID = bpi.ID AND bpi.AKTIV = '1' AND bpi.PUSER = bps.PUSER;";
  ...
  if (($sesSuperadmin == 1) || ($sesViewall == 1)){
    $frage = "SELECT * FROM bfprojektini AS bpi JOIN bfprojektstatus2 as bps WHERE bpi.AKTIV = '0' AND bpi.PUSER = bps.PUSER ORDER BY LASTDATA";    
    //$frage = "SELECT * FROM bfprojekt AS bp JOIN bfprojektstatus as bps WHERE bp.AKTIV = '0' AND bp.PUSER = bps.PUSER";
  }
  else {
   $frage = "SELECT * FROM bfuserhasprojekt AS bhp INNER JOIN bfprojektini AS bpi WHERE bhp.userID = '$sesUserid' AND bhp.projektiniID = bpi.ID AND bpi.AKTIV = '0' ORDER BY LASTDATA;";   
